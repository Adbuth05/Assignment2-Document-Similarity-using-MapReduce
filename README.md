...
- Input Key-Value Pair:
    -  Key: Byte offset of the line in the input file (ignored in our logic).
    -  Value: The entire line of text (document ID + content).

- Processing Logic:
    - The line is split into two parts: the Document ID and the document text.
    - The document text is tokenized into individual words.
    - For each word, the Mapper emits a key-value pair (word, DocumentID).
...
- Logic / Processing Steps:
    - The reducer takes a word and the list of document IDs containing that word.
    - It generates all possible pairs of documents from that list.
    - Example: if the word “Hadoop” appears in Document1, Document2, and Document3, then the reducer forms pairs: (Document1, Document2), (Document1, Document3), (Document2, Document3).
    - For each pair, it increments a counter that represents how many words those two documents have in common (this builds the intersection count).
    - Separately, the reducer also tracks the union size for each document pair, which is the total number of distinct words across both documents.
    - After processing all words, it calculates the Jaccard similarity for each document pair.  
...
- The input files are provided in the format:
``` bash
DocumentID <document text>
```
...
- Challenge: Handling large datasets with many overlapping words.
- Solution: Optimized reducer logic to compute intersections efficiently.

- Challenge: Setting up multi-node Hadoop with Docker.
- Solution: Used docker-compose.yml and hadoop.env to configure namenode, datanodes, resourcemanager, and historyserver.

- Challenge: Output overwriting when re-running jobs.
- Solution: Used unique output folder names for each run or deleted old output before execution.
